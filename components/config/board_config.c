...
void _data_to_transmit(uint8_t *buffer, size_t buffer_size,
                       size_t *tx_data_size) {
  ...
  data_to_obc.temperature_postFill =
      tanwa_data.can_sensor_temp_data.temperature1;
  ...
  data_to_obc.postRegulatorN2_pres =
      tanwa_data.can_sensor_pressure_data.pressure6;
  data_to_obc.preRegulatorN2_pres =
      tanwa_data.can_sensor_pressure_data.pressure7;
  ...
  if (buffer_size < sizeof(data_to_obc_t)) {
    ESP_LOGE(TAG, "Buffer size is too small. Required: %d, Provided: %d",
             sizeof(data_to_obc_t), buffer_size);
    return;
  }
...
ens_init_struct_t ens_init_struct = {
    ._data_to_transmit =
        _data_to_transmit, // Set to NULL for now, will be set later
    ._on_data_rx =
        ens_command_parsing, // Set to NULL for now, will be set later
...
esp_err_t board_config_init(void) {
  ...
  // ret = led_state_display_state_update(&led_state_display,
  // LED_STATE_DISPLAY_STATE_IDLE); if (ret != LED_STATE_DISPLAY_OK) {
  ...
  if (!initialize_lora(LORA_TASK_FREQUENCY_KHZ, LORA_TASK_TRANSMIT_MS)) {
    ...
    lora_set_state_periods(lora_periods,
                           sizeof(lora_periods) / sizeof(lora_periods[0]));
  }
  ...
  // SD CARD TIMER
  if (!sys_timer_start(TIMER_SD_DATA, TIMER_SD_DATA_PERIOD_MS,
                       TIMER_TYPE_PERIODIC)) {
