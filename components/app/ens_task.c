...
ens_status_t ens_init(ens_init_struct_t *init_struct,
  ...
  xTaskCreatePinnedToCore(sub_task, "ens_sub_task", 4096, NULL, 5,
                          &gb.sub_task_handle, 1);
...
static void on_recive(ens_recv_cb_data_t *recv_data) {
  ...
  if (recv_data == NULL || recv_data->data_size == 0 ||
      recv_data->data_size > MAX_DATA_LENGTH ||
      !is_from_mcb(recv_data->src_mac)) {
    return;
  }
...
static void manage_recieved_data(ens_recv_cb_data_t *recv_data) {
  ...
  if (is_broadcast(recv_data->dest_mac) &&
      recv_data->data_size == STATE_MSG_SIZE) {

    set_mission_state(recv_data);
    // ESP_LOGI(TAG, "Received broadcast message from
    ...
    // ESP_LOGI(TAG, "Command: %d, Payload: %d", recv_data->cmd.cmd.command,
    // recv_data->cmd.cmd.payload);
  } else if (is_cmd_msg(recv_data)) {

    // ESP_LOGI(TAG, "Command: %d, Payload: %d from
    // %02x:%02x:%02x:%02x:%02x:%02x",
